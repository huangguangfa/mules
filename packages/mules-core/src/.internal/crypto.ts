  ...
  Object.keys(ASYMMETRIC_CRYPTO_TYPE).forEach((prop: string) => {
    ...
    if (_item.name) {
      ...
      if (_item.hmac) {
        context[_item.name] = {
          ...context[prop],
          encrypt: (data: string, key: Record<string, string>) =>
            _crypto(data, key).toString(),
        };
      } else if (_item.params) {
        context[_item.name] = {
          ...context[prop],
          encrypt: (data: string, key: Record<string, string>) =>
            _crypto(data, key, _item.params).toString(),
        };
      } else {
        context[_item.name] = {
          ...context[prop],
    ...
    if (_item.name) {
      ...
      if (_item.encoding) {
        context[_item.name] = {
          ...context[_item.name],
          encrypt: (data: string) =>
            CryptoJS.enc[_item.name as keyof typeof CryptoJS.enc]
              .stringify(CryptoJS.enc.Utf8.parse(data))
              .toString(),
          decrypt: (data: string) =>
            CryptoJS.enc.Utf8.stringify(
              CryptoJS.enc[_item.name as keyof typeof CryptoJS.enc].parse(data)
            ),
        };
      } else {
        ...
        const { encrypt, decrypt }: any =
          CryptoJS[_item.name as keyof typeof CryptoJS];
        if (_iv) {
          context[_item.name] = {
            ...context[_item.name],
          ...
          context[_item.name] = {
            ...
            encrypt: (data: string, key: string) =>
              encrypt(CryptoJS.enc.Utf8.parse(data), _key(key)).toString(
                CryptoJS.format.Hex
              ),
...
export const getCryptoContext = (cryptoName: string) => {
  ...
  if (!_context) {
    console.error(
      `指定的加密算法[${cryptoName}]不在[ASYMMETRIC_CRYPTO_TYPE]或[SYMMETRIC_CRYPTO_TYPE]范围内！`
    );
    return false;
  }
  return _context;
