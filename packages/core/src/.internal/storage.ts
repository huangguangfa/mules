...
export const getStorageOptions = (storageType: string, cryptoType?:{name:string}): storageOptions => {
    const options = {
    ...
    return {
        ...
        encrypt: (data: any, secret: boolean = false) => secret ? Crypto.encrypt(data, cryptoType, cryptoParams) : data,
        decrypt: (data: any, secret: boolean = false) => secret ? Crypto.decrypt(data, cryptoType, cryptoParams) : data
    }
}
