...
export function parseDate(date: Date | number, format: string): string {
  ...
  for (let k in o) {
    if (new RegExp(`(${k})`).test(format)) {
      ...
      format = format.replace(
        RegExp.$1,
        RegExp.$1.length === 1 ? str : _pad(str)
      );
    }
  }
  return format;
...
export function parseNumber(
  num: number,
  { precision = 2, thousandth = false } = {}
): string {
  if (isNaN(num)) return "";
  let [temp, digit, integer, buffer, positive] = [
    0.0,
    0,
    0,
    <Array<string>>[],
  ...
  return (
    (positive ? "" : "-") +
    buffer.join(thousandth ? "," : "") +
    (precision > 0 ? "." + _zero(digit, precision) : "")
  );
}
